...
public class MarkAnalyzer {
	 ...
	 * 
	 * @author: slzs 2014-4-16 下午1:34:48
	 * @param document
	 * 
	 */
	 ...
	 * 梳理段落标记
	 * 
	 * @author: slzs 2015-2-5 下午2:04:38
	 * @param paragraphList
	 * 
	 */
	private void markComb(List<XWPFParagraph> paragraphList) {
		...
		for (int i = 0; i < paragraphList.size(); i++) {
			...
			for (int p = 0; p < runList.size(); p++) {
				...
				if (!StringUtils.isEmpty(runTempText)) {
					int markEndStrIndex;
				...
				} else if (!(markRun instanceof XWPFHyperlinkRun)) {
					paragraph.removeRun(p--); // 移除当前位置
				}
			}
		}
	 ...
	 * 清理文档${标记}
	 * 
	 * @author: slzs 2014-4-16 下午8:36:12
	 * @param document
	 */
			...
			if (bodyElement.getElementType() == BodyElementType.PARAGRAPH) {
				...
				for (XWPFTableRow row : rowList) { // 循环处理行
					...
					for (XWPFTableCell cell : cellList) { // 循环处理列
						...
						for (int pos = 0; pos < cellPghList.size(); pos++) {
							// 处理单元格内段落
	 ...
	 * 清理段落标记
	 * 
	 * @author: slzs 2015-2-12 下午2:35:11
	 * @param document
	 * @param paragraph
	 * @param 迭代key
	 * @return 是否删除空行
	...
	private boolean markClearForParagraph(XWPFDocument document, XWPFParagraph paragraph, StringBuffer iteratorKey) {
		...
		if (iteratorKey.length() > 0) {
			...
			for (int p = 0; p < runList.size(); p++) {
				...
				empty = StringUtils.isEmpty(runTextTemp); // 是否空段落
	 ...
	 * ubb标记解析
	 * 
	 * @author slzs 2017年5月9日 下午2:08:50
	 * @param document
	 * @throws Exception
	 */
