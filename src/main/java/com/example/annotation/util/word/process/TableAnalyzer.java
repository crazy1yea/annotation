...
class TableAnalyzer {
	...
	public TableAnalyzer(DataFillAnalyzer dfAnalyzer, ParagraphAnalyzer paragraphAnalyzer,
	 ...
	 * 表格标签解析
	 * 
	 * @author: slzs 2015-2-5 下午2:12:38
	 * @param document
	 * @param tableMap
	 */
	void analy(XWPFDocument document, Map<String, List<WordData>> tableMap) {
		...
		for (XWPFTable table : tableList) { // 循环表格处理
			...
			if (!ObjectUtils.isEmpty(rowDataList) && copyRow != null && !ObjectUtils.isEmpty(sourceCellList)) {
				// 根据数据复制行数
				for (int i = 1; i < rowDataList.size(); i++) {
					...
					for (int c = 0; c < toCellList.size(); c++) {
						...
						for (XWPFParagraph p : pList) {
							...
							if (!ObjectUtils.isEmpty(runList)) {
	 ...
	 * 表格数据内容处理
	 * 
	 * @author: slzs 2015-2-12 下午2:09:10
	 * @param document
	 * @param table
	 * @param data
	 */
	 ...
	 * 加载指定表格下一行数据
	 * 
	 * @author: slzs 2015-2-5 下午3:28:40
	 * @param data
	 * @param tableKey
	 * @return WordData
	 */
